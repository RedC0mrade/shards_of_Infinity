...
async def seed_all_cards():
    ...
    try:
        async with db_helper.session_context() as session:
            ...
            if existing_cards:
                print(f"✅ В базе уже есть {len(existing_cards)} карт. Пропускаем заполнение.")
                return
            
            print("Начинаем заполнение базы данных картами...")
            
            # Объединяем все данные карт
            all_cards_data = start_data + wilds_data + homodeus_cards + demirealm_cards
            ...
            for i, card_data in enumerate(all_cards_data, 1):
                try:
                    ...
                    faction = card_data["faction"]
                    card_type = card_data["card_type"]
                    print(f"  [{i}/{total_cards}] Добавлена: {card_data['name']} ({faction} - {card_type})")
                    
                except Exception as e:
                    print(f"  ❌ Ошибка при создании карты {card_data['name']}: {e}")
                    continue
    ...
    try:
        async with db_helper.session_context() as session:
            ...
            result = await session.execute(
                select(Card.faction, func.count(Card.id))
            ...
            result = await session.execute(
                select(Card.card_type, func.count(Card.id))
